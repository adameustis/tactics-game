...
public class AbilityBattleMenuController : MonoBehaviour
{
    ...
    {
        List<AbilityModel> abilityModelArray = eventModel.Tf.GetComponent<UnitBattleController>().Model.UnitAbilities;
        if (abilityModelArray == null)
            return;
        
        Display(abilityModelArray);
    }
    ...
    public void Display(List<AbilityModel> abilityModelArray)
    {
        if (IsDisplaying)
            StopDisplaying();

        TransformBackground.gameObject.SetActive(true);
        DisplayList(abilityModelArray);
    }
    ...
    public void DisplayList(List<AbilityModel> abilityModelArray)
    {
    ...
    public void AddMenuItem(AbilityModel abilityModel)
    {
    ...
    public void UpdateMenuItems(List<AbilityModel> abilityModelArray)
    {
        if(abilityModelArray == null)
            return;
        ...
        for (int i = 0; i < abilityModelArray.Count; i++)
        {
            ...
            {
                AddMenuItem(abilityModelArray[i]);
            }
        }
    }
